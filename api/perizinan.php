...
try {
    ...
    if ($action == 'stats') {
        try {
            ...
            // Get tolerance setting with fallback
            ...
            try {
                ...
                log_debug("Settings query failed, using default tolerance: " . $e->getMessage());
            }

            // Terlambat Kembali (Belum Kembali & Sudah Lewat jam rencana kembali)
            // Konsisten dengan badge di frontend - tanpa toleransi
            $stmt = $pdo->query("
                SELECT COUNT(*) as cnt FROM perizinan 
                WHERE waktu_kembali IS NULL 
                AND NOW() > rencana_kembali
            ");
            $late = $stmt->fetch()['cnt'];

            // Recent Activity (termasuk asrama)
            $stmt = $pdo->query("
            SELECT p.*, s.nama, s.kelas, s.asrama 
            FROM perizinan p 
    ...
    if ($action == 'create' && $method == 'POST') {
        ...
        // Cek duplikasi izin aktif - santri yang belum kembali
        $stmt = $pdo->prepare("SELECT id FROM perizinan WHERE santri_nis = ? AND waktu_kembali IS NULL");
        $stmt->execute([$nis]);
        if ($stmt->fetch()) {
